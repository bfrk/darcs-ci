...
unset \
 DARCS_DONT_ESCAPE_ISPRINT DARCS_USE_ISPRINT\
 DARCS_ESCAPE_8BIT\
...
test_line $(printf '\x01\x02\x03\x04\x05\x06\x07\x08\x0B\x0C\x0E')\
          '[_^A_][_^B_][_^C_][_^D_][_^E_][_^F_][_^G_][_^H_][_^K_][_^L_][_^N_]'
# this doesn't seem to work on windows, no idea why:
if ! os_is_windows; then
  test_line $(printf '\x0D') '[_^M_]'
fi
...
# everything up to here is escaped by default;
# for the rest we must explicitly set it:
export DARCS_ESCAPE_8BIT=1
