...
rm -rf temp1
make_repo_with_test
if darcs test --linear --set-scripts-executable | grep 'Test does not fail on head' ; then
    echo "ok 5"
else
...
if grep 'Test failed!' trackdown-out && grep 'Last recent patch' trackdown-out ; then
    echo "ok 7"
else 
    echo "not ok 7 either no failure or no report of problem patch (both should occur)"
    exit 1
fi
cd ..
