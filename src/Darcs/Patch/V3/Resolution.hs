...
import Darcs.Patch.V3.Contexted ( Contexted, ctxId, ctxToFL )
...
The result differs in two aspects from that for RepoPatchV1:

  * We do not remove contexted patches that some other alternative in the
    component depends on. We prefer to be more verbose and more explicit
    here, giving the user more choices.

  * We do not determine the maximal independent sets for each component and
    neither do we merge these (non-conflicting) alternatives. While the
    latter gives a theoretically valid and more compact presentation of
    conflicting alternatives, the merging of obscures where these
    alternatives originally came from.

    This will become important when we add prim patch IDs to the markup to
    indicate the origin of each alternative.
...
findComponents context patches = go S.empty [] [] context patches NilFL where
  ...
  go todo done res _ NilRL _
    | S.null todo = sort $ foldr joinOverlapping done res
