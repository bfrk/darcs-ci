...
overlay base over = Tree {items = M.fromList immediate, treeHash = NoHash}
  where
    ...
    get n =
      case (M.lookup n $ items base, M.lookup n $ items over) of
        ...
        (Just (Stub b _), Just (SubTree o)) -> Stub (overlay <$> b <*> pure o) NoHash
        (Just (SubTree b), Just (Stub o _)) -> Stub (overlay <$> pure b <*> o) NoHash
        (Just (Stub b _), Just (Stub o _)) -> Stub (overlay <$> b <*> o) NoHash
        (Just x, _) -> x
