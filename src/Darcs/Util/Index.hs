  ...
  where
    ...
    readDir item = do
      ...
      let substate = substateof item state
          want =
            exists && (predicate index) (path substate) (Stub undefined NoHash)
          oldhash = iHash' item
          subs off =
...
dumpIndex indexpath =
  ...
  where
    ...
    readEntry p = do
      ...
      ieHash <-
        SHA256 . BS.fromShort <$> BS.createFromPtr (p `plusPtr` off_hash) size_hash
