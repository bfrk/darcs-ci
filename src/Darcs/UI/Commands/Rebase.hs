...
doSuspend opts _repository suspended psToSuspend = do
    ...
    _repository <-
      tentativelyRemovePatches _repository NoUpdatePending psToSuspend
    new_suspended <-
...
unsuspendCmd cmd reifyFixups _ opts _args =
  withRepoLock (useCache ? opts) (umask ? opts) $ RebaseJob $ \_repository -> do
    ...
    _repository <-
      tentativelyAddPatches _repository (verbosity ? opts)
...
obliterateCmd _ opts _args =
  withRepoLock (useCache ? opts) (umask ? opts) $ RebaseJob $ \_repository -> do
    ...
    void $ finalizeRepositoryChanges _repository YesUpdatePending (O.dryRun ? opts)
...
applyPatchesForRebaseCmd cmdName opts _repository (Fork common us' to_be_applied) = do
    ...
    Sealed pw <-
        tentativelyMergePatches
            ...
            (allowConflicts opts)
            (externalMerge ? opts)
            (wantGuiPause opts) (verbosity ? opts)
