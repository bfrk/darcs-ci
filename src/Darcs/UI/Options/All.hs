...
remoteDarcs = imap (Iso fw bw)
  $ singleStrArg [] ["remote-darcs"] F.RemoteDarcsOpt arg "COMMAND"
    "name of the darcs executable on the remote server"
  where
    arg (F.RemoteDarcsOpt s) = Just s
    arg _ = Nothing
    fw k (RemoteDarcs y) = k (Just y)
    fw k DefaultRemoteDarcs = k Nothing
    bw k (Just y) = k (RemoteDarcs y)
    bw k Nothing = k DefaultRemoteDarcs
...
data ShrinkFailure = ShrinkFailure | NoShrinkFailure deriving (Eq, Show)

shrinkFailure :: PrimDarcsOption ShrinkFailure
shrinkFailure = withDefault ShrinkFailure
  [ RawNoArg [] ["shrink-failure"] F.ShrinkFailure ShrinkFailure
      "try to cut down the set of patches causing a test failure"
  , RawNoArg [] ["no-shrink-failure"] F.NoShrinkFailure NoShrinkFailure
